...
class Enemie:
    ...
    def update(self):
        ...
        dist = round(((screen_info.current_h//2+coord_sous_fenetre[0]-self.coords[0])**2+(screen_info.current_w//2+coord_sous_fenetre[1]-self.coords[1])**2)**0.5)
        t=[None, dist]
...
def update_enemies():
    ...
    list_en_dist = []
    kill = False
    for en in range(len(liste_enemies)):
        if liste_enemies[en-int(kill)].life <= 0:
            del liste_enemies[en-int(kill)]
            kill = True
        else:
            t = liste_enemies[en-int(kill)].update()
            list_dist.append(t[1])
            list_en_dist.append(liste_enemies[en-int(kill)])
            if not touche and t[0] is not None:
                touche = True
                _t = t[0]
            liste_enemies[en-int(kill)].display()
    if list_dist != []:
        en = min(list_dist)
